# PRISM Research Claim Validation Matrix

**Framework Version:** 1.3.2  
**Date:** 03 September 2026  
**Auditor:** Independent Scientific Audit & Reproduction Pipeline  
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :---: |
| **Claim 1: Multi-Feature Hybrid Candidate Matching** | Combining lexical, semantic, skills, experience, and education signals provides richer candidate representation than pure keyword matching. | `experiments/models.py:PRISMHybridPipeline`, `lib/matching/score.ts` | `datasets/sample_benchmark.jsonl` ($N=20$) | `experiments/run.py --config baseline.yaml` | Accuracy, Precision, Recall, F1 Score | Accuracy = 0.6667, F1 = 0.6667 (Mean continuous score difference over keyword: +0.0504) | **PARTIALLY VALIDATED** *(Algorithmic implementation verified; generalizable superiority over baselines requires $N \ge 1,000$ external dataset)* |
...
| **Claim 3: Demographic Fairness Auditing & Parity Monitoring** | Post-prediction threshold monitoring evaluates disparate impact across protected demographic cohorts (Groups A–C). | `experiments/metrics.py`, `lib/fairness/engine.ts`, `app/fairness` | Synthetic demographic cohorts ($N=20$) | `experiments/run.py --config fairness.yaml` | Selection Rate, DPD, EOD, DIR | Baseline DPD = 1.0000 on holdout ($N=3$); calibrated mitigation reduces parity delta by 50% | **PARTIALLY VALIDATED** *(Auditing math and threshold mitigation execute as designed; real-world fairness claims require external demographic hiring data)* |
...
| **Claim 5: Component Utility via Architectural Ablation** | Each module in the PRISM pipeline incrementally improves or stabilizes matching performance. | `experiments/evaluator.py`, `experiments/run.py --config ablation.yaml` | `datasets/sample_benchmark.jsonl` ($N=20$) | Ablation Benchmark | Accuracy, F1, DPD, EOD across 5 configurations | Lexical only ($0.3333 \text{ Acc}$) $\rightarrow$ Full PRISM ($0.6667 \text{ Acc}$, $0.6667 \text{ F1}$) | **PARTIALLY VALIDATED** *(Ablation pipeline executes; sample size $N=3$ holdout provides limited statistical power)* |
| **Claim 6: Real-Time Computational Efficiency** | Hybrid feature extraction and candidate scoring can execute in low-latency environments. | `experiments/reproduce.py` (100 iterations) | Benchmark holdout cohort | Empirical Latency Profiling | Mean, P50, P95, P99 Latency (ms) | Mean Warm Total = 6.48 ms, P95 = 10.22 ms, Per-Candidate = 2.16 ms/cand | **VALIDATED** *(Empirically measured over 100 repeated runs)* |
| **Claim 7: Zero Data Leakage Across Partitions** | Disjoint applicant splitting prevents candidate identity and resume text contamination between training and evaluation partitions. | `experiments/dataset.py:split_dataset` | Benchmark dataset ($N=20$, seed 42) | Disjoint Applicant Set Intersection Test | $\text{Train} \cap \text{Test} = \emptyset$ | $\text{Intersection} = 0$ candidate identities; 0 duplicate resumes across partitions | **VALIDATED** *(No leakage detected in audited pipeline)* |
